...
if (!function_exists('rigolettres_get_seo_data')) {
    function rigolettres_get_seo_data() {
        ...
        if (is_front_page()) {
            ...
            $data['description'] = 'Jeux et livres créés par Brigitte, orthophoniste depuis 25 ans à Mamers. Méthode syllabique ludique pour enfants de 5 à 12 ans. Fabriqués en France. Livraison offerte dès 60 €.';
            ...
            $data['title'] = $title . ' | ' . $brand;
            $data['description'] = $excerpt ?: "Rigolettres : jeux éducatifs créés par Brigitte, orthophoniste à Mamers depuis 25 ans.";
            $thumb_id = get_post_thumbnail_id();
            if ($thumb_id) {
                $img = wp_get_attachment_image_src($thumb_id, 'large');
                if ($img) { $data['og_image'] = $img[0]; $data['og_image_width'] = $img[1]; $data['og_image_height'] = $img[2]; }
